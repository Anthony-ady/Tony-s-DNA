/**
 * Company Analytics Page Component
 * 
 * This page provides detailed analytics for a specific company using the Druid API endpoint.
 * It uses the AnalyticsTemplate for common functionality and provides company-specific logic.
 */
...
import { DollarSign, TrendingUp, BarChart3, Calendar, ChevronDown, ChevronUp, Loader2 } from "lucide-react";
...
export default function CompanyAnalytics() {
  ...
  const buildPayload = (companyId, beginDateISO, endDateISO, viewMode) => {
    ...
    if (viewMode === 'hourly') {
      ...
      const adjustedYesterday = new Date(yesterday.getTime());
        ...
        processedData.push({
          ...
          dspRevenueTrend: yesterdayItem ? (todayItem.PriceAdvertiser_PublisherSide > yesterdayItem.PriceAdvertiser_PublisherSide ? 'up' :
            todayItem.PriceAdvertiser_PublisherSide < yesterdayItem.PriceAdvertiser_PublisherSide ? 'down' : 'same') : 'same',
          dspRevenueChangePercent: yesterdayItem && yesterdayItem.PriceAdvertiser_PublisherSide > 0 ?
          ...
          publisherCostsTrend: yesterdayItem ? (todayItem.PricePublisher > yesterdayItem.PricePublisher ? 'up' :
            todayItem.PricePublisher < yesterdayItem.PricePublisher ? 'down' : 'same') : 'same',
          publisherCostsChangePercent: yesterdayItem && yesterdayItem.PricePublisher > 0 ?
            (((todayItem.PricePublisher - yesterdayItem.PricePublisher) / yesterdayItem.PricePublisher) * 100).toFixed(1) : 0,
          marginTrend: yesterdayItem ? ((todayItem.PriceAdvertiser_PublisherSide - todayItem.PricePublisher) > (yesterdayItem.PriceAdvertiser_PublisherSide - yesterdayItem.PricePublisher) ? 'up' :
          ...
          marginChangePercent: yesterdayItem && (yesterdayItem.PriceAdvertiser_PublisherSide - yesterdayItem.PricePublisher) !== 0 ?
  ...
  const calculateSummaryStats = (data, viewMode) => {
    ...
    return {
      ...
      avgMarginPercentage: marginPercentage,
      dataPoints: data.length
    };
  };

  // Fetch hourly data for a specific date
  const fetchHourlyDataForDate = async (dateString, companyId) => {
  ...
  const toggleDateExpansion = async (dateString, dateKey, companyId) => {
    ...
    if (newExpandedDates.has(key)) {
      ...
      if (!hourlyDataCache[key] && !loadingHourlyData.has(key)) {
        ...
        try {
          ...
          const hourlyData = await fetchHourlyDataForDate(dateToFetch, companyId);
  ...
  const renderTableRow = (item, index, analyticsData, viewMode, showDetailedColumns, entityId = null) => {
    ...
    // Calculate trends (comparing with previous item)
                      ...
                      const dspRevenueChangePercent = prevItem && prevItem.PriceAdvertiser_PublisherSide > 0 ? 
                        Math.abs(((dspRevenue - prevItem.PriceAdvertiser_PublisherSide) / prevItem.PriceAdvertiser_PublisherSide) * 100).toFixed(1) : 0;
                      const publisherCostsChangePercent = prevItem && prevItem.PricePublisher > 0 ? 
                        Math.abs(((publisherCosts - prevItem.PricePublisher) / prevItem.PricePublisher) * 100).toFixed(1) : 0;
                      const marginChangePercent = prevItem && (prevItem.PriceAdvertiser_PublisherSide - prevItem.PricePublisher) !== 0 ? 
