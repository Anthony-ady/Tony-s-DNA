...
// Metric options organized by categories based on network_operations datasource reference
const METRIC_OPTIONS = [
  // Core Auction Flow Metrics
  { label: 'Bid Requests', value: 'network_operations_bid_requests', category: 'Core Auction Flow' },
  { label: 'Bid Responses', value: 'network_operations_bid_responses', category: 'Core Auction Flow' },
  { label: 'Bid Under Floor', value: 'network_operations_bid_under_floor', category: 'Core Auction Flow' },
  { label: 'Bid Won', value: 'network_operations_bid_won', category: 'Core Auction Flow' },
  { label: 'Win Rate', value: 'network_operations_win_rate', category: 'Core Auction Flow' },
  { label: 'Loss', value: 'network_operations_loss', category: 'Core Auction Flow' },
  // Delivery & Quality Metrics
  { label: 'Inventory', value: 'network_operations_inventory', category: 'Delivery & Quality' },
  { label: 'Impressions', value: 'network_operations_impressions', category: 'Delivery & Quality' },
  { label: 'Visible Impressions', value: 'network_operations_visible_impressions', category: 'Delivery & Quality' },
  { label: 'Viewability Rate', value: 'network_operations_viewability_rate', category: 'Delivery & Quality' },
  { label: 'Click', value: 'network_operations_click', category: 'Delivery & Quality' },
  // Financial Metrics
  { label: 'Price Publisher', value: 'network_operations_price_publisher', category: 'Financial' },
  { label: 'Price Advertiser', value: 'network_operations_price_advertiser', permission: 'SUPER ADMIN', category: 'Financial' },
  { label: 'eCPM Publisher', value: 'network_operations_ecpm_publisher', category: 'Financial' },
  { label: 'eCPM Advertiser', value: 'network_operations_ecpm_advertiser', permission: 'SUPER ADMIN', category: 'Financial' },
  { label: 'USD Win Price', value: 'network_operations_usd_win_price', permission: 'SUPER ADMIN', category: 'Financial' },
  // Label Metrics (Optional Enrichment)
  { label: 'Placement Name', value: 'PlacementName', category: 'Display' },
  { label: 'Site Name', value: 'SiteName', category: 'Display' },
  { label: 'Publisher Name', value: 'PublisherName', category: 'Display' },
  { label: 'Advertiser Name', value: 'AdvertiserName', category: 'Display' },
  { label: 'Deal Name', value: 'DealName', category: 'Display' },
  { label: 'Agency Name', value: 'AgencyName', category: 'Display' },
  { label: 'Campaign Name', value: 'CampaignName', category: 'Display' },
  { label: 'Partner Name', value: 'PartnerName', category: 'Display' },
];

const DIMENSION_OPTIONS = [
  { label: 'Ad Kind', value: 'adKind' },
  { label: 'Device', value: 'assDevice' },
  { label: 'Deal', value: 'dealId' },
  { label: 'Placement', value: 'placementId' },
  { label: 'Site', value: 'siteId' },
  { label: 'Seat', value: 'seatId' },
  { label: 'Publisher', value: 'publisherId' },
  { label: 'Realm', value: 'realmId' },
];
...
const FILTER_FIELDS = [
  ...
  { label: 'Deal', value: 'dealId' },
  { label: 'Site', value: 'siteId' },
  { label: 'Company', value: 'publisherId' },
  { label: 'Placement', value: 'placementId' },
  { label: 'Seat', value: 'seatId' },
  { label: 'Ad Kind', value: 'adKind' },
  { label: 'Device', value: 'assDevice' },
...
// Map UI field names to API field names for network_operations payload
...
const FILTER_OPERATORS = ['in', 'match', 'equals', 'contains'];

const DEFAULT_CONFIG = {
  metrics: METRIC_OPTIONS.slice(0, 3),
  dimensions: [],
  filters: [],
  granularity: 'all', // 'all' for aggregated data, 'daily' for day by day
};
...
function formatValue(value, columnName = '', granularity = 'daily') {
  ...
  if (typeof value === 'number') {
    // For network_operations, values are already in the correct format (no division needed)
    // Format Price and eCPM columns as currency (values are already in dollars)
    const columnNameLower = columnName ? columnName.toLowerCase() : '';
    // Check if this is a network_operations financial metric (price, ecpm, usd_win_price)
    const isNetworkOperationsFinancial = columnName && (
      columnName.startsWith('network_operations_price') ||
      columnName.startsWith('network_operations_ecpm') ||
      columnName.startsWith('network_operations_usd_win_price') ||
      (columnNameLower.includes('network_operations') && (columnNameLower.includes('price') || columnNameLower.includes('ecpm') || columnNameLower.includes('usd_win_price')))
    );
    
    if (isNetworkOperationsFinancial) {
      // Values are already in dollars, just format as currency
...
function formatTimestamp(value, granularity) {
  ...
  // For BuilderOperations, granularity is 'all' (no timestamp) or 'daily' (YYYY-MM-DD)
  // Format as YYYY-MM-DD for daily granularity
  ...
  const [state, setState] = useState(() => {
    ...
    try {
      ...
      return {
        ...
        granularity: parsed.granularity === 'all' || parsed.granularity === 'daily' ? parsed.granularity : DEFAULT_CONFIG.granularity,
      };
    } catch (error) {
...
export default function BuilderOperations() {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (sessionData?.CurrentUser) {
          ...
          setIsPowerUser(sessionData.CurrentUser.IsPowerUser || false);
        }
      } catch (error) {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `builder-operations-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const exportToXML = () => {
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `builder-operations-${new Date().toISOString().split('T')[0]}.xml`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const exportToJSON = () => {
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `builder-operations-${new Date().toISOString().split('T')[0]}.json`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const handleAddMetric = (metric) => {
    setState((prev) => ({
      ...prev,
      metrics: prev.metrics.some((item) => item.value === metric.value)
        ? prev.metrics
        : [...prev.metrics, metric],
    }));
  };
  ...
  const runQuery = async () => {
    ...
    // At least one metric is required (dimensions and filters are optional)
    ...
    state.filters.forEach((filter) => {
      ...
      // Map UI field name to API field name for network_operations
    ...
    // Use granularity from state: 'all' for aggregated data, 'daily' for day by day
    const granularity = state.granularity === 'daily' 
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1 max-w-2xl">
            Data is available up to three months back and up to yesterday (no same-day data).
          </p>
        </div>
              ...
              <div className="flex flex-wrap gap-4 items-end">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() => setState((prev) => ({ ...prev, granularity: 'all' }))}
                      className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${
                        state.granularity === 'all'
                    ...
                    <button
                      type="button"
                      onClick={() => setState((prev) => ({ ...prev, granularity: 'daily' }))}
                      className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${
...
function MetricSelectionDialog({ open, onOpenChange, selectedMetrics, onMetricsChange, userRank, isPowerUser }) {
  ...
  // Filter available metrics based on user permissions
  ...
  // Get category order
  const categoryOrder = ['Financial', 'Core Auction Flow', 'Delivery & Quality', 'Display'];
...
function SelectDialogs({ open, onOpenChange, options, onSelect, title, description }) {
  if (!open) return null;
...
function FilterDialog({ open, onOpenChange, filters, onFiltersChange }) {
  ...
  const [selectedRealms, setSelectedRealms] = useState([]); // Store selected realms separately
  ...
  const [dealPopoverOpen, setDealPopoverOpen] = useState({});
  ...
  // Load data when dialog opens (all searches are now dynamic, loaded on popover open)
  useEffect(() => {
    if (open) {
      // No initial loading - all searches are now dynamic and triggered when popovers open
    }
      ...
      const updatedFilters = prev.map((filter) => {
        if (filter.id === id) {
          ...
          if (field === 'field' && filter.field !== value) {
            ...
            } else if (oldField === 'publisherId') {
              setCompanySearchTerms((prev) => {
                const newState = { ...prev };
                delete newState[id];
                return newState;
              });
            } else if (oldField === 'SiteId') {
              setSiteSearchTerms((prev) => {
                const newState = { ...prev };
                delete newState[id];
                return newState;
              });
            } else if (oldField === 'PlacementId') {
              setPlacementSearchTerms((prev) => {
                const newState = { ...prev };
                delete newState[id];
                return newState;
              });
            } else if (oldField === 'DealId') {
              setDealSearchTerms((prev) => {
                const newState = { ...prev };
                delete newState[id];
                return newState;
              });
  ...
  const handleRealmToggle = (filterId, realmId) => {
    ...
    if (realmIds.includes(realmId)) {
      ...
      // Remove from selectedRealms if no longer selected in any filter
      ...
      // Add to selectedRealms if not already there
  ...
  const getSelectedRealmNames = (filterId) => {
    ...
    return realmIds.map(id => {
      // Check in both realms (search results) and selectedRealms (previously selected)
      const realm = realms.find(r => r.Uid === id) || selectedRealms.find(r => r.Uid === id);
      return realm ? realm.Name : id;
    });
  };

  // Debounced search for realms - trigger API call when search term changes for open popovers
  useEffect(() => {
    const timeouts = {};
  ...
  }, [realmSearchTerms, realmPopoverOpen]); // eslint-disable-line react-hooks/exhaustive-deps

  // Initialize search when popover opens for realm
  useEffect(() => {
    Object.keys(realmPopoverOpen).forEach((filterId) => {
      if (realmPopoverOpen[filterId]) {
  ...
  }, [realmPopoverOpen]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  }, [companySearchTerms, companyPopoverOpen]); // eslint-disable-line react-hooks/exhaustive-deps

  // Initialize search when popover opens for company
  useEffect(() => {
    Object.keys(companyPopoverOpen).forEach((filterId) => {
      if (companyPopoverOpen[filterId]) {
  ...
  }, [companyPopoverOpen]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  }, [siteSearchTerms, sitePopoverOpen]); // eslint-disable-line react-hooks/exhaustive-deps

  // Initialize search when popover opens for site
  useEffect(() => {
    Object.keys(sitePopoverOpen).forEach((filterId) => {
      if (sitePopoverOpen[filterId]) {
  ...
  }, [sitePopoverOpen]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    const timeouts = {};
    Object.keys(dealPopoverOpen).forEach((filterId) => {
      if (dealPopoverOpen[filterId]) {
        const searchTerm = dealSearchTerms[filterId] || '';
        clearTimeout(timeouts[filterId]);
        timeouts[filterId] = setTimeout(() => {
  ...
  }, [dealSearchTerms, dealPopoverOpen]); // eslint-disable-line react-hooks/exhaustive-deps

  // Initialize search when popover opens for deal
  useEffect(() => {
    Object.keys(dealPopoverOpen).forEach((filterId) => {
      if (dealPopoverOpen[filterId]) {
        const currentSearchTerm = dealSearchTerms[filterId] || '';
        fetchDeals(currentSearchTerm);
      }
    });
  }, [dealPopoverOpen]); // eslint-disable-line react-hooks/exhaustive-deps

  const getFilteredRealms = (filterId) => {
    // Merge search results with selected realms to ensure selected ones are always visible
    const filter = localFilters.find(f => f.id === filterId);
    const selectedIds = filter && filter.values ? filter.values.split(',').map(v => v.trim()).filter(Boolean) : [];
    
    // Get selected realms that might not be in current search results
  ...
  const handleCompanyToggle = (filterId, companyId) => {
    ...
    if (companyIds.includes(companyId)) {
      ...
      // Remove from selectedCompanies if no longer selected in any filter
      const stillSelected = localFilters.some(f => {
        if (f.id === filterId) return false;
        const values = f.values ? f.values.split(',').map(v => v.trim()).filter(Boolean) : [];
      ...
      // Add to selectedCompanies if not already there
  ...
  const getSelectedCompanyNames = (filterId) => {
    ...
    return companyIds.map(id => {
      // Check in both companies (search results) and selectedCompanies (previously selected)
  ...
  const getFilteredCompanies = (filterId) => {
    // Merge search results with selected companies to ensure selected ones are always visible
    const filter = localFilters.find(f => f.id === filterId);
    const selectedIds = filter && filter.values ? filter.values.split(',').map(v => v.trim()).filter(Boolean) : [];
    
    // Get selected companies that might not be in current search results
  ...
  const handleSiteToggle = (filterId, siteId) => {
    ...
    if (siteIds.includes(siteId)) {
      ...
      // Remove from selectedSites if no longer selected in any filter
      const stillSelected = localFilters.some(f => {
        if (f.id === filterId) return false;
        const values = f.values ? f.values.split(',').map(v => v.trim()).filter(Boolean) : [];
      ...
      // Add to selectedSites if not already there
  ...
  const getSelectedSiteNames = (filterId) => {
    ...
    return siteIds.map(id => {
      // Check in both sites (search results) and selectedSites (previously selected)
  ...
  const getFilteredSites = (filterId) => {
    // Merge search results with selected sites to ensure selected ones are always visible
    const filter = localFilters.find(f => f.id === filterId);
    const selectedIds = filter && filter.values ? filter.values.split(',').map(v => v.trim()).filter(Boolean) : [];
    
    // Get selected sites that might not be in current search results
  ...
  // Deal functions
  const handleDealToggle = (filterId, dealId) => {
    const filter = localFilters.find(f => f.id === filterId);
    if (!filter) return;

    const currentValues = filter.values ? filter.values.split(',').map(v => v.trim()).filter(Boolean) : [];
    const dealIds = currentValues;
    
    let newDealIds;
    if (dealIds.includes(dealId)) {
      newDealIds = dealIds.filter(id => id !== dealId);
      // Remove from selectedDeals if no longer selected in any filter
      const stillSelected = localFilters.some(f => {
        if (f.id === filterId) return false;
        const values = f.values ? f.values.split(',').map(v => v.trim()).filter(Boolean) : [];
        return values.includes(dealId);
      });
      if (!stillSelected) {
        setSelectedDeals(prev => prev.filter(d => d.Uid !== dealId));
      }
    } else {
      newDealIds = [...dealIds, dealId];
      // Add to selectedDeals if not already there
      const deal = deals.find(d => d.Uid === dealId);
      if (deal) {
        setSelectedDeals(prev => {
          if (prev.some(d => d.Uid === dealId)) return prev;
          return [...prev, deal];
        });
      }
    }

    updateFilter(filterId, 'values', newDealIds.join(', '));
  };

  const getSelectedDealNames = (filterId) => {
    const filter = localFilters.find(f => f.id === filterId);
    if (!filter || !filter.values) return [];
    
    const dealIds = filter.values.split(',').map(v => v.trim()).filter(Boolean);
    return dealIds.map(id => {
      // Check in both deals (search results) and selectedDeals (previously selected)
      const deal = deals.find(d => d.Uid === id) || selectedDeals.find(d => d.Uid === id);
      return deal ? deal.Name : id;
    });
  };

  const getFilteredDeals = (filterId) => {
    // Merge search results with selected deals to ensure selected ones are always visible
    const filter = localFilters.find(f => f.id === filterId);
    const selectedIds = filter && filter.values ? filter.values.split(',').map(v => v.trim()).filter(Boolean) : [];
    
    // Get selected deals that might not be in current search results
    const selectedDealsList = selectedDeals.filter(d => selectedIds.includes(d.Uid));
    
    // Merge and remove duplicates
    const allDeals = [...deals, ...selectedDealsList];
    const uniqueDeals = Array.from(
      new Map(allDeals.map(deal => [deal.Uid, deal])).values()
    );
                ...
                <div className="space-y-2">
                  <Label>
                    {filter.field === 'realmId' ? 'Select Realms' :
                     ...
                     filter.field === 'PlacementId' ? 'Select Placements' :
                     filter.field === 'DealId' ? 'Select Deals' :
                     'Values (comma-separated)'}
                  </Label>
                  {filter.field === 'realmId' ? (
                    <Popover 
                    ...
                    <Popover 
                      open={dealPopoverOpen[filter.id] || false} 
                      onOpenChange={(open) => setDealPopoverOpen(prev => ({ ...prev, [filter.id]: open }))}
                    >
                      <PopoverTrigger asChild>
                        <Button
                          variant="outline"
                        ...
                        >
                          <div className="flex flex-wrap gap-1 flex-1">
                            {getSelectedDealNames(filter.id).length > 0 ? (
                              getSelectedDealNames(filter.id).map((name, idx) => (
                                <Badge key={idx} variant="secondary" className="text-xs">
                                  {name}
                                </Badge>
                              ))
                            ) : (
                              <span className="text-slate-500 text-xs">Select deals...</span>
                            )}
                          </div>
                          <ChevronDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                        </Button>
                      ...
                      <PopoverContent className="w-[400px] p-0" align="start">
                        <Command>
                          <CommandInput
                            ...
                            value={dealSearchTerms[filter.id] || ''}
                            onValueChange={(value) => setDealSearchTerms(prev => ({ ...prev, [filter.id]: value }))}
                            className="h-9"
                          />
                          <CommandList>
                            <CommandEmpty>No deal found.</CommandEmpty>
                            <CommandGroup>
                              {getFilteredDeals(filter.id).map((deal) => {
                                const isSelected = filter.values && filter.values.split(',').map(v => v.trim()).includes(deal.Uid);
                                return (
                                  <CommandItem
                                    key={deal.Uid}
                                    value={deal.Name}
                                    onSelect={() => handleDealToggle(filter.id, deal.Uid)}
                                    className="text-xs"
                                  >
                                    <div className="flex items-center gap-2 flex-1">
                                      <div
