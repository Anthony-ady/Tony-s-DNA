/**
 * Broker Profitability Page Component
 * 
 * Displays network operations data grouped by RealmPublisher
 */
...
import placementBrokerMapping from '../Broker/placement-broker-mapping.json';

export default function BrokerProfitability() {
  ...
  const [allowedRealmIds, setAllowedRealmIds] = useState(new Set());
  ...
  // Extract unique realmIds from placement-broker-mapping.json
  useEffect(() => {
    const uniqueRealmIds = new Set(placementBrokerMapping.map(item => item.realmId).filter(Boolean));
    setAllowedRealmIds(uniqueRealmIds);
  }, []);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Process data - aggregate by realm (since we use P1D granularity)
      const aggregated = {};
      ...
      dataArray.forEach(item => {
        const realmUid = item.realm || item.realmId; // API returns "realm" not "realmId"
        if (!realmUid) return;
        
        // Filter: only include realms that are in placement-broker-mapping.json
        if (allowedRealmIds.size > 0 && !allowedRealmIds.has(realmUid)) return;
        
        if (!aggregated[realmUid]) {
          aggregated[realmUid] = {
            realmId: realmUid,
            realmName: null, // Realm name not available from network_operations API
        ...
        aggregated[realmUid].network_operations_bid_requests += item.network_operations_bid_requests || 0;
        aggregated[realmUid].network_operations_bid_responses += item.network_operations_bid_responses || 0;
        aggregated[realmUid].network_operations_impressions += item.network_operations_impressions || 0;
        aggregated[realmUid].network_operations_price_publisher += item.network_operations_price_publisher || 0;
        aggregated[realmUid].network_operations_price_advertiser += item.network_operations_price_advertiser || 0;
  ...
  }, [startDate, endDate, allowedRealmIds]);
  ...
  const filteredData = data.filter(item => {
    ...
    const realmName = getRealmName(item.realmId).toLowerCase();
    const realmId = item.realmId.toLowerCase();
    const searchLower = searchTerm.toLowerCase();
    return realmName.includes(searchLower) || realmId.includes(searchLower);
  });
  ...
  const sortedData = [...filteredData].sort((a, b) => {
    ...
    switch (sortConfig.key) {
      case 'realmName':
        aValue = getRealmName(a.realmId);
        bValue = getRealmName(b.realmId);
        break;
      case 'rpbr':
  ...
  const exportToCSV = () => {
    ...
    const rows = sortedData.map((item, index) => {
      const realmName = getRealmName(item.realmId);
    ...
    link.setAttribute('download', `broker-profitability-${new Date().toISOString().split('T')[0]}.csv`);
              ...
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead 
                    ...
                    onClick={() => handleSort('realmName')}
                  >
                    <div className="flex items-center gap-1">
                      <span>Realm Publisher</span>
                      {sortConfig.key === 'realmName' ? (
              ...
              <TableBody>
                {paginatedData.length === 0 ? (
                  ...
                  paginatedData.map((item, index) => (
                    <TableRow key={item.realmId} className="hover:bg-[rgb(75,99,226)]/10">
                        ...
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-gradient-to-br from-[rgb(75,99,226)] to-[rgb(75,99,226)] rounded-lg flex items-center justify-center">
                            <span className="text-white text-sm font-bold">
                              {getRealmName(item.realmId).charAt(0).toUpperCase()}
                            </span>
                          </div>
                          <div>
                            <div 
                              ...
                              onClick={() => {
                                const realmName = getRealmName(item.realmId);
                                navigate(`/RealmAnalytics?id=${item.realmId}&name=${encodeURIComponent(realmName)}`);
                              }}
                            >
                              {getRealmName(item.realmId)}
                            </div>
      ...
      {totalCount > 0 && (
        <div className="border-t border-slate-200 bg-white px-3 flex items-center justify-between flex-shrink-0 sticky bottom-0 z-10 shadow-sm w-full left-0 right-0" style={{ height: '80px' }}>
          <div className="text-sm text-slate-600">
            Showing {startIndex + 1} to {Math.min(endIndex, totalCount)} of {totalCount} {totalCount === 1 ? 'realm' : 'realms'}
          </div>
